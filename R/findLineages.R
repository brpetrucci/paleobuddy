...
#'
#' @return A \code{list} object with (named) sim objects corresponding to the
#' clades originated from species in S.
#'
#' @examples
#'
...
findLineages <- function(sim, S=NULL) {
  ...
  for (s in S) {
    final[[paste0("clade_", s)]] = helper_findLineage(sim, s)
  }
  return(final)
}
