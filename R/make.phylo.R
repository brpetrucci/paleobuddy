...
#'
#' @param return.root.time Logical indicating if phylo should have information
...
#'
#' @details When root.time is added to a phylogeny, packages as ape can change their
...
#' practices in its default value for \code{return.root.time} by automatically 
...
#' sim=list(TE=NA)
#' while(is.na(max(sim$TE)) | length(sim$TE)==1){
#'   sim <- bd.sim(n0 = 1, lambda = 0.2, mu = 0.05, tMax = 10)
#' }
#' 
#' sim
...
#' phy_rootless <- make.phylo(sim, return.root.time = FALSE)
#' 
#' #plot the phylogeny:
#' par(mfrow = c(1, 3))
#' # plot them
