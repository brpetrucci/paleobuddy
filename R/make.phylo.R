...
#' species' edge (i.e. a sampled ancestor tree, sensu Heath et al, (2014)).
#' 
#' Note this package does not
#' depend on APE (Paradis et al, 2004) since it is never used inside its 
#' functions, but it is suggested since one might want to manipulate the 
#' phylogenies generated by this function.
#' 
...
#'              nFinal = c(2, Inf), extOnly = TRUE)
#' 
#' # make the phylogeny (Birth-Death only)
#' phy <- make.phylo(sim)
#' 
#' #Now, lets simulate a fossilized Birth-Death process
#' #sample fossils:
#' fossils <-  sample.clade(sim, 0.1, 10)
#' while (nrow(fossils) < 2) fossils <- sample.clade(sim,
#'  0.1, 10) #needed just to provide an example more easily
#' 
#' # make the phylogeny (Fossilized Birth-Death)
#' FBDphy <- make.phylo(sim, fossils)
#' 
#' par(mfrow = c(1, 2))
#' # plot it
#' if (requireNamespace("ape", quietly = TRUE)) {
#'   ape::plot.phylo(phy, main="Only Birth-Death")
#'   ape::axisPhylo()
#'   
#'   # we can also plot only the molecular phylogeny
#'   ape::plot.phylo(FBDphy, main="Fossilized Birth-DEath")
#'   ape::axisPhylo()
#' }
#' 
#' @name make.phylo
...
make.phylo <- function(sim, fossils=NULL) {
  ...
  #if fossils are provided, change sim object to describe a Fossilized-Birth-Death process:
  if(!is.null(fossils)){
    
    if(nrow(fossils)==0){
      stop("Please insert a data frame containig fossil data. See help(make.phylo) for more information")
    }
    ...
    sampledSpecies <- which(sim$EXTANT)
    ...
    sampledNames <- paste0("t", sampledSpecies)
  ...
  } #end of adequating sim object for describing FBD
