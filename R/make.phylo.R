...
make.phylo <- function(sim, fossils = NULL, saFormat = "branch",
  ...
  # if there are fossils, correct tree as needed
  if (!is.null(fossils)) {
    # if we don't want true extinction times, drop extinct species
    if (!returnTrueExt)
      phy <- ape::drop.tip(phy, extinctTips)
    
    # make 0-length branches degree-2 nodes if saFormat is "node"
    if (saFormat == "node") {
      # tips to delete, i.e. ones where edge is of length 0
      delInd <- phy$edge.length == 0
      
      # get the tips to delete
      deletedTips <- phy$edge[delInd, 2]
      
      # labels of tips to delete
      internalLabels <- phy$tip.label[deletedTips]
      
      # parents of those tips
      saParents <- phy$edge[delInd, 1]
      
      # record depths of SA parents before dropping tips
      sa_depths <- ape::node.depth.edgelength(phy)[saParents]
      
      # drop tips
      phy <- ape::drop.tip(phy, deletedTips, collapse.singles = FALSE)
      ...
      n_children <- tabulate(phy$edge[, 1] - new_ntips, nbins = phy$Nnode)
      degree2 <- which(n_children == 1) + new_ntips
      
      # iterate through internal labels
      for (i in seq_along(internalLabels)) {
        # get the new node number for this node
        pi_new <- degree2[which.min(abs(new_depths[degree2] - sa_depths[i]))]
        
        # rename it
        phy$node.label[pi_new - new_ntips] <- internalLabels[i]
      }
    }
    
    # check again if we want to return true extinction times
    if (!returnTrueExt) {
