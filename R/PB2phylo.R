...
#'
#' \code{PB2phylo} generates a \code{phylo} object using a simulation from the 
...
#' lineage.
#'
#' @param \code{sim} a simulation coming from the \code{BDSim} function.
#' 
    ...
    if(is.numeric(dau) & length(dau)>0){ #if lineage has daughters
      ...
      if(jump==1){ #if a whole clade has very recently being put in the phylogeny
        ...
        if(is.na(edge[nrow(edge),2])){
          ...
          edge=rbind(edge,
                     matrix(nrow = 1, ncol = 2, data=c(prev.node, cur.node))) # eh aqui!! o no 13-15 tbm passa aqui
        }
