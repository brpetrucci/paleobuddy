...
parse.nexus <- function(file, treeNumbers = NULL) {
  # Read file
...
extract.translate.block.nexus <- function(file) {
  # read file
  lines <- readLines(file)
...
parse.newick.annotations <- function(newick, taxonDiction) {
  ...
  # last tidying:
  rownames(df) <- NULL
  df <- df[!df$name %in% c("tree", "MCC_tree"), ]
  df <- df[-grep("STATE_", df$name), ]
...
build.buddPhylo <- function(newick, parsedTxt, tolExtant = 10E-10) {
  ...
  for (ss in SAncNames) {
    ...
    branches$y_coord[SAID] <- branches$y_coord[SAParID] + rnorm(n = 1, sd = .3)
...
read.nexus <- function(file, treeNumbers = NULL, timeFromRoot = FALSE) {
  
  # parse file info & build buddPhylo:
  txt <- parse.nexus(file, treeNumbers = treeNumbers)
  if (length(txt) > 1) {
    ...
    for (i in 1:length(txt)) {
      phy <- build.buddPhylo(newick = txt[[i]], parsedTxt = as.data.frame(branches[i]))
      res[[i]] <- adjust.timescale.buddPhylo(phy)
    }
    class(res) <- "multi.buddPhylo"
  } else {
    txt <- txt[[1]]
    branches <- parse.newick.annotations(newick = txt,
                    taxonDiction= extract.translate.block.nexus(file))
    phy <- build.buddPhylo(newick = txt, parsedTxt = branches)
