...
Let us first try the simplest possible birth-death scenario - constant speciation and extinction rates.
...
The output of `bd.sim` is a `sim` object, a class made up of named vectors that is organized as follows
  ...
  * `PAR` a list of parents. The naming of species follows the order of `TE` and `TS`, i.e. if `PAR[i] == j`, the species whose speciation time is `TS[j]` generated species `i`. For species alive at the beginning of the simulation, the parent is `NA`.
...
For illustration, we create a simulation with more than 500 tips, with 253 extant tips.
...
## Linear speciation and extinction
...
# returnTrue = FALSE makes it so the occurrences are binned (e.g. an occurrence
# at time 42.34 is returned as between 42 and 41)
...
# left no fossils. In this case, it was 1
...
The latter two columns would be substituted by `SampT` if `returnTrue = TRUE`, as default.
...
# draw longevities with fossil ranges
draw.sim(simHead, fossils = samp)
```
...
sim <- suppressMessages(bd.sim(n0, lambda, mu, tMax, mShape = mShape, envL = temp))
...
sim <- suppressMessages(bd.sim(n0, lList, q, tMax, lShape = lShape, 
                               mShape = mShape, envM = temp, lShifts = lShifts))
